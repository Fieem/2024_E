...
- `--white-value-min`：白棋亮度下限，默认 `155`
- `--white-saturation-max`：白棋饱和度上限，默认 `80`
- `--white-norm-value-min`：白棋归一化亮度下限，默认 `135`
...
`NEW` 用于开始新的 PLACE 流程，会清空已使用的取子槽位和已请求的落子位置，并返回 `BUSY,RESET`。
...
- `READY`：读取当前稳定整盘状态，若轮到机械臂，则调用 `gomoku_ai` 的 `hard` 难度求下一步，根据当前 `theta_deg` 做落子旋转补偿，再返回 4 个脉冲
  - 记录上一轮机械臂落子后的预期棋盘
  - 如果发现任意棋子从原位置移动到了另一个空位置，会先发送校正脉冲将其复原，不要求移动前后的颜色识别完全一致；本次 READY 不调用 AI
  - 棋盘恢复稳定后，再次发送 READY 才继续计算下一步
  - 无法识别的多处变化返回 `ERROR,ILLEGAL_BOARD,ILLEGAL BOARD`
...
- `NEW` 会同时清空 PLACE 的取子槽位和落子位置记录
- READY 对弈模式未指定取子序号，仍按当前棋盘中该颜色棋子数量选择下一个槽位

倾斜补偿配置：

- `tilt_compensation.enabled`
  - `true`：开启 PLACE 和 READY 模式下的落子倾斜补偿
  - `false`：PLACE 和 READY 模式直接使用基准格脉冲
...
- `tilt_compensation.max_extrapolation_cells`
  - 脉冲表边界最大斜率外推距离，单位为棋盘格，默认 `1.25`
  - 超过该距离时返回 `TILT_OUT_OF_RANGE`，不再使用边界值截断
...
- 目标点在表内时使用双线性插值；轻微越界时沿边界两点的脉冲斜率外推
- 外推距离超过 `max_extrapolation_cells` 时返回 `TILT_OUT_OF_RANGE`
